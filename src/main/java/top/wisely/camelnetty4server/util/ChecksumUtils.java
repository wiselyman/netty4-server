...
public class ChecksumUtils {
    ...
    private static int[] table = { 0x0000, 0x1189, 0x2312, 0x329b, 0x4624,
            ...
            0xd3a5, 0xe13e, 0xf0b7, 0x0840, 0x19c, 0x2b52, 0x3adb, 0x4e64,
            ...
            0x2c6a, 0x1ef1, 0x0f78 }; // crc table


    public static int calcCRC(byte[] cp) {
        int fcs = 0xffff;
        int i = 0;
        while (i < cp.length){
            fcs = (fcs >> 8) ^ table[(fcs ^ cp[i++]) & 0xff];
        }
        return fcs;
    }

    public static int calcCRC(List<Byte> cp) {
        int fcs = 0xffff;
        int i = 0;
        while (i < cp.size())
        {
            fcs = (fcs >> 8) ^ table[(fcs ^ cp.get(i++)) & 0xff];
        }
        return fcs;
    }
